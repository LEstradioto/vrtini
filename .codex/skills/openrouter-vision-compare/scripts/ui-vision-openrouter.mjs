...
function estimateVerticalOffset(baselinePng, testPng, requestedMaxShift) {
  const safeMaxShift = Math.max(
    0,
    Math.min(requestedMaxShift, Math.floor(Math.min(baselinePng.height, testPng.height) * 0.2))
  );
  ...
  for (let shift = -safeMaxShift; shift <= safeMaxShift; shift += 1) {
    ...
    const overlap = Math.min(baselinePng.height - baselineStart, testPng.height - testStart);
...
function prepareCompareChunks({
  ...
  const verticalOffset = estimateVerticalOffset(baselinePng, testPng, alignMaxShift);
...
function aggregateCompareChunks(chunkResults, maxIssues, route) {
  ...
  const averageConfidence = chunkResults.length
    ? Number(
        (
          chunkResults.reduce((sum, chunk) => sum + (chunk.confidence ?? 0.6), 0) /
          chunkResults.length
        ).toFixed(2)
      )
    : 0;
...
async function runCompareMode({ args, route, model, maxIssues, timeoutMs, seed }) {
  ...
  const useDiff = parseBoolean(args.useDiff ?? process.env.UI_VISION_USE_DIFF ?? '0', false);
...
async function run() {
  ...
  const model =
    args.model || process.env.UI_VISION_MODEL || process.env.OPENROUTER_MODEL || FALLBACK_MODEL;
  ...
  if (payload.mode === 'compare') {
    ...
    console.log(
      `Diff helper image: ${payload.useDiff && payload.diffPath ? 'enabled' : 'disabled'}`
    );
    console.log(`Estimated vertical offset: ${payload.verticalOffset ?? 0}px`);
    console.log(
      `Aggregate verdict: ${payload.aggregate.verdict} (${payload.aggregate.confidence})`
    );
