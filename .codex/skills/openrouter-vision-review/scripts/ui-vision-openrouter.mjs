...
async function run() {
  ...
  if (!hasReportPath && !explicitImagePath) {
    throw new Error(
      'report.json not found. Pass --report or generate ui-design-pipeline output first.'
    );
  }
  ...
  if (DRY_RUN) {
    normalizedIssues = [
      {
        ...
        recommendation:
          'Disable dry-run and set OPENROUTER_API_KEY in project-root .env for real vision analysis.',
        confidence: 0.99,
      },
    ];
    ...
    if (!apiKey) {
      throw new Error(
        'OPENROUTER_API_KEY is missing. Add it to the project root .env or export it in your shell.'
      );
    }
